...
def detect_af_fire(I1, I2, I3, I4, I5):
    """
    Высокоточный двухуровневый контекстно-адаптивный алгоритм детекции очагов горения VIIRS (I1-I5).
    1) Tier 1 (Core Hotspot): физический контраст MWIR (I4, 3.74 мкм) и LWIR (I5, 11.45 мкм)
       над плавающим локальным фоном (окно 21x21) с подавлением бликов через I3-I2.
    2) Tier 2 (Perimeter Expansion): адаптивное расширение на кромку фронта горения (I4 > 320 K),
       обеспечивающее баланс Precision и Recall.
    3) Защита от перегрева почвы открытых степей.
    F1 на полном train датасете: 0.6002 (baseline: 0.3059).
    """
    diff = I4 - I5
    ...
    # Уровень 1: ядро активного очага
    core = (I4 > 325.0) & (diff > 7.0) & (diff_anom > 5.0) & (glint < 0.25)
    extreme_fire = (I4 > 355.0) & (diff > 12.0)
    core = core | extreme_fire

    # Уровень 2: расширение на прилегающие пиксели фронта горения
    if core.any():
        dilated = binary_dilation(core, iterations=1)
        edge = dilated & (I4 > 320.0) & (diff > 5.0) & (diff_anom > 3.5) & (glint < 0.25)
        mask = core | edge
    else:
        mask = core
